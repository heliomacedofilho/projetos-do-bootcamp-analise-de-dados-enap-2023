import streamlit as st


st.set_page_config(page_title="Equipe", page_icon="🏥", layout="wide")
st.title('_Equipe_ :hospital:')
st.markdown("---")

st.write("## Projeto Final do Bootcamp de Análise de Dados - ENAP")
st.write("### Turma Exclusiva para Mulheres - Outubro/2023")


st.markdown(
    """
    <p style='text-align: justify;'>
    Esse projeto foi desenvolvido a partir de uma colaboração harmoniosa de uma 
    equipe de servidoras públicas talentosas e dedicadas. Cada participante 
    trouxe sua experiência única e conhecimentos para o projeto, contribuindo com 
    diferentes perspectivas e habilidades. Essa abordagem coletiva permitiu-nos 
    enfrentar desafios complexos em tempo hábil, possibilitando o desenvolvimento e 
    entrega de dois projetos independentes.
    </p>
    <h5 style='text-align: justify;'>
    Daiana Sales
    </h5>
    <p style='text-align: justify;'>
    Possui graduação em Licenciatura pela Universidade do Estado do Rio de Janeiro (2011) e Mestrado em Geofísica pelo Observatório Nacional (2014). Atualmente é técnica de geociências do Serviço Geológico do Brasil SGB/CPRM. Tem experiência na área de Geociências, com ênfase em Geofísica Aplicada, atuando principalmente nos seguintes temas: geofísica, inversão e desenvolvimento de software. Atualmente é coordenadora da Rede de Litotecas da SGB/CPRM e Membro da Diretoria da ABMGeo-RJ.
    </p>
    <p style='text-align: justify;'>
    [LINK DO GITHUB OU LINKEDIN](url)
    </p>
    
    <h5 style='text-align: justify;'>
    Harlane Magalhães
    </h5>
    <p style='text-align: justify;'>
    Administradora na Fundação Nacional do Índio desde 2004. Possui especialização em Direito Público e em Gestão Tributária e Auditoria no Serviço Público com experiência nas área de: Orçamento e Finanças, Modernização e organização, Planejamento, Gestão Estratégica, Mapeamento de Processos, Gestão de Projetos, Compras, Licitações e Contratos, Auditoria, Gestão de Pessoas e Logística.
    </p>
    <p style='text-align: justify;'>
    <a href='https://www.linkedin.com/in/harlane-magalh%C3%A3es-39443a196/'> Linkedin</a>
    </p>

    <h5 style='text-align: justify;'>
    Michela Camboim
    </h5>
    <p style='text-align: justify;'>
    Pesquisadora, economista, cientista de dados e desenvolvedora Full Stack Jr do 
    Núcleo de Estudos Econômicos e Populacionais da Fundação Joaquim Nabuco/Ministério 
    da Educação. Possui doutorado em economia pela UFPE, e desenvolve trabalhos de 
    análise de dados com uso de estatística descritiva e inferencial, algoritmos de 
    Machine Learning e uso ferramentas como Python, Linguagem R, DAX, Power BI, 
    SQL e outras. Como pesquisadora, desenvolve projetos nas áreas de Economia Social, Economia 
    da Educação, Economia Regional e Economia do Trabalho, atuando principalmente nos 
    seguintes temas: educação, avaliação de políticas públicas educacionais, saúde, 
    desigualdades sociais.
    </p>
    <p style='text-align: justify;'>
    <a href='https://www.https://github.com/MichelaCamboim/'> Github</a>
    </p>
    <p style='text-align: justify;'>
    <a href='https://www.linkedin.com/in/michela-camboim/'> Linkedin</a>
    </p>

    <h5 style='text-align: justify;'>
    Paula Bittencourt
    </h5>
    <p style='text-align: justify;'>
    Psicóloga servidora do Ministério da Saúde. Possui Especialização em Gestão de Pessoas, Gestão de Projetos de Software e Gestão de Pessoas, Inovação e Resultados. Realizou cursos nas áreas de controle, auditoria, gestão de riscos, controle social e institucional, gestão da qualidade, planejamento estratégico, ciência de dados e desenvolvimento web full-stack. Atua na área de controle interno e auditoria, exercendo atividades relacionadas à análise de solicitações de órgãos de controle, ao mapeamentos dos processos, ao planejamento estratégico, ao monitoramento da execução do planejamento anual e ao uso de dados e informações para a gestão estratégica da auditoria do SUS.
    </p>
    <p style='text-align: justify;'>
    <a href='https://github.com/pbitgomes'> Github</a>
    </p>

    <h5 style='text-align: justify;'>
    Priscilla Uchoa
    </h5>
    <p style='text-align: justify;'>
    Mestra pelo POLEDUC (Mestrado Profissional em Políticas Públicas e Gestão da Educação Superior), programa da Universidade Federal do Ceará, e graduada em Pedagogia pela mesma instituição. Especialista em Docência para a Educação Profissional e Tecnológica pelo Instituto Federal de Educação, Ciência e Tecnologia do Espírito Santo. Bacharel em Direito e Especialista em Direito das Famílias pela Universidade Regional do Cariri. Tem experiência nas áreas de Direito e Educação, com ênfase em Direitos Humanos e Educação Profissional e Tecnológica. Atualmente é servidora técnico administrativa do Instituto Federal de Educação, Ciência e Tecnologia do Ceará, no Campus de Sobral, atuando como Assistente da Diretoria de Ensino. Recentemente desenvolveu interesse pela linguagem Python, a partir das turmas específicas para mulheres servidoras públicas.
    </p>
    <p style='text-align: justify;'>
    <a href='https://github.com/PriscillaUchoa'> Github</a>
    </p>

    <h5 style='text-align: justify;'>
    Rosana Pinho
    </h5>
    <p style='text-align: justify;'>
    Possui graduação em Farmácia e Bioquímica pela Universidade Federal do Paraná (UFPR), Mestrado e    Doutorado em Biologia Celular e Molecular pela UFPR, com período sanduíche em Max-Delbrück-Centrum em Berlim. Experiência em monitoramento e avaliação de políticas públicas, com ênfase em análise de bancos de dados com foco em gestão e construção de indicadores monitoráveis. Atualmente trabalha na área de Assessoria de Informação do Departamento de HIV/aids do Ministério da Saúde, onde é responsável pelas informações de monitoramento clínico de pessoas vivendo com HIV e da resistência aos antirretrovirais, com publicações periódicas por relatórios e painéis interativos.
    </p>
    <p style='text-align: justify;'>
    
    </p>


    """,
    unsafe_allow_html=True
)
